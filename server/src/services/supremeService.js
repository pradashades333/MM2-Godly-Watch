...
async function scrapeGodlies() {
  try {
    const response = await fetch("https://supremevalues.com/mm2/godlies");
...
async function scrapeChromas() {
  try {
    const response = await fetch("https://supremevalues.com/mm2/chromas");
    const html = await response.text();
    const $ = cheerio.load(html);
    ...
    console.error("scrapeChromas failed", err);
    return [];
  }
}
